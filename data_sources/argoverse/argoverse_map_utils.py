...
# Copy from https://github.com/jagjeet-singh/argoverse-forecasting/blob/27696a19b3753a4ad8dcae3ad9c6107c5e4e237c/utils/map_features_utils.py
...
from argoverse.utils.centerline_utils import get_nt_distance, remove_overlapping_lane_seq
...
class MapFeaturesUtils:
    ...
    def get_point_in_polygon_score(
        ...
        lane_seq_polygon = cascaded_union(
            [Polygon(avm.get_lane_segment_polygon(lane, city_name)).buffer(0) for lane in lane_seq]
        )
    ...
    def sort_lanes_based_on_point_in_polygon_score(
        ...
        sorted_point_in_polygon_scores_idx = np.lexsort((randomized_tiebreaker, np.array(point_in_polygon_scores)))[
            ::-1
        ]
        sorted_lane_seqs = [lane_seqs[i] for i in sorted_point_in_polygon_scores_idx]
        sorted_scores = [point_in_polygon_scores[i] for i in sorted_point_in_polygon_scores_idx]
    ...
    def get_heuristic_centerlines_for_test_set(
        ...
        num_diverse_centerlines = min(len(diverse_centerlines), max_candidates - aligned_cl_count)
        test_centerlines = aligned_centerlines
        if num_diverse_centerlines > 0:
            probabilities = (
                [float(score + 1) / (sum(diverse_scores) + len(diverse_scores)) for score in diverse_scores]
    ...
    def get_candidate_centerlines_for_trajectory(
        ...
        mahattan_threshold: float = 5.0,
        dfs_front_scale: float = 20.0,
        dfs_back_scale: float = 20.0,
        mode: str = "test",
    ) -> List[np.ndarray]:
        ...
        Args:
            ...
            max_candidates: Maximum number of centerlines to return,
            mahattan_threshold: threshold to find nearby lanes,
            dfs_front_scale: front scale when performing dfs traversing,
            dfs_back_scale: back scale when performing dfs traversing,
        ...
        curr_lane_candidates = avm.get_lane_ids_in_xy_bbox(xy[-1, 0], xy[-1, 1], city_name, mahattan_threshold)

        # Keep expanding the bubble until at least 1 lane is found
        while len(curr_lane_candidates) < 1 and mahattan_threshold < max_search_radius:
            mahattan_threshold *= 2
            curr_lane_candidates = avm.get_lane_ids_in_xy_bbox(xy[-1, 0], xy[-1, 1], city_name, mahattan_threshold)
        ...
        dfs_threshold_front = dfs_front_scale * (seq_len + 1 - traj_len) / 10
        dfs_threshold_back = dfs_back_scale * (traj_len + 1) / 10
        ...
        obs_pred_lanes, scores = self.sort_lanes_based_on_point_in_polygon_score(obs_pred_lanes, xy, city_name, avm)
